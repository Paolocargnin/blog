...
| Draft without a source ledger                | `$blog-report`          | Build private claim-to-source provenance.                            |
...
self-approval. An agent pauses so the named human can inspect the exact files
and expressly authorize the command. Publication still finishes through a
reviewed pull request and human merge.

## Evidence contract

`sources.md` is the exhaustive private ledger. `fact-check.md` records a fresh,
independent check and reaches `status: passed` only when every item is resolved.
...
`## Sources` section, with a rationale either way. It also records whether a
...
verification markers, unresolved counter-discussion findings, and inconsistent
Sources or disclosure decisions.
