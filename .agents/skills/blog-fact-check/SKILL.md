...
2. Dispatch a fresh checker subagent or separate human who did not draft the
   article. Give it the raw `article.md`, `sources.md`, and `fact-check.md`
   paths—no proposed verdict, suspected answer, or drafting conclusions.
...
4. Let the Author apply fixes. Recheck the final wording. Set
   `independent: true` and `status: passed` only when every checklist item and
   finding is resolved and no verification marker remains.
