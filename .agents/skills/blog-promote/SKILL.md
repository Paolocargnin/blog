---
name: blog-promote
description: Promote a content-complete Draft to Publication candidate when its argument, metadata, controlled tags, reporting, and prose are stable enough for verification-only changes.
---
...
2. Confirm the Draft is content-complete: public metadata is valid, required
   reporting is complete, known gaps are explicit, and structural or
   substantive work is finished. A private source ledger is optional for a
   purely personal piece. Run `pnpm content:check`.
